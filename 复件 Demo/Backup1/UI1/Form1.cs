...
namespace UI1
{
    public partial class Form1 : Form
    {
        ...
        {
            string serchStr=textBox2.Text.ToLower();
            
            List<MyFileInfo> newList=new List<MyFileInfo>();
            for (int i=0; i < list.Count; i++)
                if (list[i].DirectoryName.ToLower().Contains(serchStr)||list[i].FileName.ToLower().Contains(serchStr)||list[i].Mark.ToLower().Contains(serchStr))
                    newList.Add(list[i]);
